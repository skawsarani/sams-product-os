...
description: Extracts action items from recent meeting transcripts, deduplicates against existing tasks, and presents findings for review before creating. Invoked via /process-meetings or "process my meeting notes". Supports parallel processing for 3+ transcripts.
allowed-tools: list_tasks, check_duplicates, create_task, Glob, Read, Write, Bash(qmd:*)
...
- Processing log: `meetings/.processed-meetings`
...
```
Glob: meetings/**/2026-02-10-*.md
Glob: meetings/**/2026-02-09-*.md
Glob: meetings/**/2026-02-08-*.md
```

Run all date globs in parallel for speed. Do NOT use `meetings/**/*.md` (matches all transcripts and will truncate).

### Step 1.5: Filter Already-Processed

Read `meetings/.processed-meetings`. Remove any found transcripts that are already listed. Report: "Found X transcripts, Y already processed, Z new." If zero new, stop early and tell the user.
...
Compare extracted items against existing tasks in `tasks/`, initiatives in `initiatives/`.
...
Append all processed transcript paths (relative to `meetings/`) to `meetings/.processed-meetings`, one per line.

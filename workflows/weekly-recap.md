---
allowed-tools: list_tasks, get_task_summary, find_stale_tasks, Glob, Read, Write
argument-hint: [optional: "quick" for condensed, "save" to persist, "slack" for Slack-optimized]
description: Generate a weekly recap of key initiatives for sharing with manager and execs
---
...
- Transcripts are in `knowledge/transcripts/` with YYYY-MM-DD date prefixes.
- Initiatives are in `knowledge/initiatives/`.
- Voice guide for exec tone is in `VOICE-GUIDE.md`.
- Output location (if saved): `knowledge/weekly-recaps/YYYY-MM-DD.md`
...
- "save" → After generating the full recap, write output to `knowledge/weekly-recaps/YYYY-MM-DD.md`
- "slack" → Format a Slack-optimized version (shorter, plain markdown, no nested headers)
- Multiple arguments can be combined (e.g., "quick save")
...
**Actions:**
1. Use Glob to find files in `knowledge/transcripts/` with date prefixes within the date range from Step 0
...
3. Read any files in `knowledge/initiatives/` for additional context
...
## Step 7: Save and Offer Persistence

If `$ARGUMENTS` contains "save":
- Write the full recap to `knowledge/weekly-recaps/YYYY-MM-DD.md` (using the Monday date from Step 0)

If `$ARGUMENTS` does NOT contain "save":
- After presenting the recap, ask: "Want me to save this to `knowledge/weekly-recaps/`?"
